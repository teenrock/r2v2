function botmp(message,prefix,client,fs,path){
  
let warns = JSON.parse(fs.readFileSync(path.resolve(__dirname, ("../../data/botmp.json"))));
if (message.content.startsWith(prefix + "botmp")){
      ...
      if (message.mentions.users.size != 0) {
        if (args[0] === "<@!"+mentioned.id+">"||args[0] === "<@"+mentioned.id+">") {
          if (args.slice(1).length != 0) {
            ...
            fs.writeFile(path.resolve(__dirname, ("../../data/botmp.json")), JSON.stringify(warns), (err) => {if (err) console.error(err);});
message.delete();
...
message.mentions.users.first().send(args.slice(1).join(' '))
  ...
  if (message.content.startsWith(prefix+"viewbotmp")||message.content===prefix+"viewbotmp") {
if (message.channel.type === "dm") return;
if(!message.guild.member(message.author).hasPermission("MANAGE_GUILD")) return message.reply("**:x: Vous n'avez pas la permission `Modérateur` sur ce serveur**").catch(console.error);
    const mentioned = message.mentions.users.first();
    const args = message.content.split(' ').slice(1);
      ...
      if (message.mentions.users.size !== 0) {
        if (args[0] === "<@!"+mentioned.id+">"||args[0] === "<@"+mentioned.id+">") {
          ...
          for (var warn in warns[message.guild.id][mentioned.id]) {
            ...
            "\"** envoyé par **"+message.guild.members.find("id", warns[message.guild.id][mentioned.id][warn].user).user.tag+"** le **"+warns[message.guild.id][mentioned.id][warn].time+"**");
          }
          message.channel.send(arr.join('\n'));
        } else {
          message.channel.send("Erreur mauvais usage: "+prefix+"viewbotmp <user> <message>");
          console.log(args);
        }
      } else {
        message.channel.send("Erreur mauvais usage: "+prefix+"viewbotmp <user> <message>");
  ...
  if (message.content.startsWith(prefix+"clearbotmp")||message.content===prefix+"clearbotmp") {
if (message.channel.type === "dm") return;
if(!message.guild.member(message.author).hasPermission("MANAGE_GUILD")) return message.reply("**:x: Vous n'avez pas la permission `Modérateur` sur ce serveur**").catch(console.error);
   const mentioned = message.mentions.users.first();
    const args = message.content.split(' ').slice(1);
      ...
      if (message.mentions.users.size != 0) {
        if (args[0] === "<@!"+mentioned.id+">"||args[0] === "<@"+mentioned.id+">"){
          if (!isNaN(arg2)) {
            ...
            message.channel.send("Erreur mauvais usage: "+prefix+"clearbotmp <utilisateur> <nombre|tout>");
          }
        } else {
          message.channel.send("Erreur mauvais usage: "+prefix+"clearbotmp <utilisateur> <nombre|tout>");
        }
