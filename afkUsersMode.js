function afkUsersMode(message, prefix, fs, path, decache) {

  const staff_cmd = bot.channels.get('434136167181713409');
  const general = bot.channels.get('434133682509447179');
  const tchat_cmd = bot.channels.get('434134476663160832');
  const cmd_adm = bot.channels.get('490060949294940160');
  const users_cmd = bot.channels.get("494543618296643591");
  const xzdc_chantest = bot.channels.get('494532982091153408');
  const xzdc = bot.users.get('399094992338944012');
  ...
  if (message.content.startsWith(prefix + 'afk')) {
    ...
    if ((msgchan != xzdc_chantest)&&(msgchan != cmd_adm)&&(msgchan != users_cmd)) {
      message.reply('cette commande est restreinte au salon : **' + users_cmd + '**\n Veuillez renouveler la commande sur ce salon.')
    } else {
...
Si malgré tout vous n'y parvenez pas, vous pouvez essayer de réinitialiser votre profilà l'aide de la commande : **!afk reset**`);
    ...
    afk(message, prefix, bot, args, afkMSGpres, fs, path, decache)
  }
  }
